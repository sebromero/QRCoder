...
public class QRCodeGenerator : NSObject {
	...
	private func imageWithImageFilter(inputImage:CIImage) -> CIImage {
		if let colorFilter = CIFilter(name: "CIFalseColor") {
			...
			if #available(OSX 10.10, *) {
				...
				return (colorFilter.valueForKey("outputImage") != nil) ?   colorFilter.valueForKey("outputImage") as! CIImage : CIImage()
			}
		}
		return CIImage()
	}
	...
	public func createImage(value:String, size:CGSize) -> QRImage {
		...
		if let qrFilter = CIFilter(name: "CIQRCodeGenerator") {
			...
			if #available(OSX 10.10, *) {
				outputImage = imageWithImageFilter(qrFilter.outputImage!)
			} else {
				let ciimage = (qrFilter.valueForKey("outputImage") != nil) ?   qrFilter.valueForKey("outputImage") as! CIImage : CIImage()
				outputImage = imageWithImageFilter(ciimage)
			}
